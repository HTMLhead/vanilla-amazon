...
class SearchAutocorrect {
  ...
  init() {
    this.fetchUrl = this.urlObj.fetchUrl;
    ...
    this.formSubmitPrevent(qs('#input-form'));
    ...
    this.autocorrectWindow.addEventListener('click', e =>
      this.goAddress(e.target)
    );
  ...
  getSearchData(e) {
    ...
    fetch(this.fetchUrl + inputValue).then(res => {
  ...
  changeListBackgourndColorUp(list) {
    ...
    if (bFirstData) {
      list[this.autocorrectListIndex + 1].classList.remove(
        this.optionObj.shadingAutocorrectClass
      );
      return;
    }
    list[this.autocorrectListIndex].classList.add(
      this.optionObj.shadingAutocorrectClass
    );
    list[this.autocorrectListIndex + 1].classList.remove(
      this.optionObj.shadingAutocorrectClass
